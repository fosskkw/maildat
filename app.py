...
def main():
    ...
    df = pd.read_csv(csv)
    email_key: str = typer.prompt("Email key in CSV", default="Email")
    df.drop_duplicates(subset=[email_key])
    
    template = typer.prompt("Email template:", default="./data/email.temp.md")
    ...
    title_template = typer.prompt("Email title template")
    ...
    typer.prompt("Press enter to open template preview", default=" ", show_default=False, hide_input=True)
    webbrowser.open_new_tab("file://" + sample_path)

    typer.confirm("Do you want to continue and send emails", default=False, abort=True)
